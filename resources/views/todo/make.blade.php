...
@section('body')
    ...
    @endif
    <form action="{{route('todo.makeAPI')}}" method="post" class="form-todo-make">
        @csrf
        <label for="input_title" class="label-title" >Title</label>
        <input id="input_title" name="title" class="input-title" placeholder="Choose your title of todo" value="{{$attributes['title'] ?? old('title')}}" required/>

        <label for="textarea_description" class="label-description">Description</label>
        <textarea id="textarea_description" name="description" class="textarea-description custom-scroll" rows="5" placeholder="Give descriptions..." required>{{$attributes['description'] ?? old('description')}}</textarea>

        <label for="input_due" class="label-due">Due (time)</label>
        <input id="input_due" name="due" type="datetime-local" class="input-due" placeholder="Set the time should to be done" value="{{$attributes['due'] ?? old('due')}}"/>
        ...
        <input id="input_commander" name="commander" type="email" list="list_users" class="input-commander" placeholder="Set the commander (whos make this todo)" title="unique name must be started with @ character" value="{{$attributes['commander'] ?? old('commander')}}" autocomplete="off" required/>
        ...
        <input id="input_soldier" name="soldier" type="email" list="list_users" class="input-soldier" placeholder="Set your soldier (whos will get this todo)" title="unique name must be started with @ character" value="{{$attributes['soldier'] ?? old('soldier')}}" autocomplete="off" required/>
