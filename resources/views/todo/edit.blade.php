...
@section('body')
    ...
    <form action="{{route('todo.update',[$todo])}}" method="post" class="form-todo-make">
        ...
        <input id="input_title" name="title" class="input-title @error('title') input-validation-error  @enderror" placeholder="Choose your title of todo" value="{{$attributes['title'] ?? old('title') ?? $todo->getAttribute('title') }}" required/>
        @error('title')
        ...
        <textarea id="textarea_description" name="description" class="textarea-description custom-scroll @error('description') input-validation-error  @enderror" rows="5" placeholder="Give descriptions..." required>{{$attributes['description'] ?? old('description') ?? $todo->getAttribute('description')}}</textarea>
        @error('description')
            <div class="todo-message message-validation-error">
                <h2>{{$message}}</h2>
            </div>
        ...
        <input id="input_soldier" name="soldier" type="email" list="list_users" class="input-soldier @error('soldier') input-validation-error  @enderror" placeholder="Set your soldier (whos will get this todo)" title="unique name must be started with @ character" value="{{$attributes['soldier'] ?? old('soldier') ?? $soldier ?? null}}" autocomplete="off" required/>
        @error('soldier')
            <div class="todo-message message-validation-error">
                <h2>{{$message}}</h2>
            </div>
